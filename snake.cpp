...
void Snake::add_body(int cnt, QGraphicsScene *scene){
    for(int i = 0; i < cnt; i++){
        ...
        if(body[body_parts-1]->x() > body[body_parts-2]->x())
        {
            body[body_parts]->setPos(body[body_parts-2]->x()+20, body[body_parts-2]->y());
        }
        else if(body[body_parts-1]->x() < body[body_parts-2]->x())
        {
            body[body_parts]->setPos(body[body_parts-2]->x()-20, body[body_parts-2]->y());
        }
        else if(body[body_parts-1]->y() > body[body_parts-2]->y())
        {
            body[body_parts]->setPos(body[body_parts-2]->x(), body[body_parts-2]->y()+20);
        }
        else if(body[body_parts-1]->y() < body[body_parts-2]->y())
        {
            body[body_parts]->setPos(body[body_parts-2]->x(), body[body_parts-2]->y()-20);
